...
Re-platforming CBATechno from a single Next.js monolith (self-hosted) to **one
TanStack Start web client (customer + role-gated vendor/admin) + an Expo mobile app
on Supabase**. Full rationale and the original implementation plan:
`~/.claude/plans/1-mobile-scope-humming-boot.md`.
...
| `cbatechno-shared/` | `@cbatechno/shared` pkg | built; consumed via `file:` link |
| `cbatechno-mobile/` | Expo SDK 56 | scaffold + Supabase/Query/auth + magic-link deep-linking |
